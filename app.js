...
let watchlist = new Set(
  JSON.parse(localStorage.getItem(LS_WATCHLIST_KEY) || "[]")
);
...
document.getElementById("tabs").addEventListener("click", (e) => {
  ...
  document
    ...
    .classList.toggle("hidden", activeTab !== "crypto");
  document
    .getElementById("fundraisingSection")
    .classList.toggle("hidden", activeTab !== "fundraising");
});
...
btnTopGainers?.addEventListener("click", () => {
  sortKey = "change24h";
  sortDir = "desc";
  if (sortSelect) sortSelect.value = "change24h:desc";
  renderCrypto();
});
btnTopLosers?.addEventListener("click", () => {
  sortKey = "change24h";
  sortDir = "asc";
  if (sortSelect) sortSelect.value = "change24h:asc";
  renderCrypto();
});
...
function openCoinModal(asset) {
  ...
  coinModalMeta.innerHTML = `
    ...
    Market Cap: <strong>${formatNum(asset.marketCap)}</strong> • 
    Sector: ${asset.sector || "-"}</div>`;
  ...
  document.getElementById("swapSimBtn")?.addEventListener("click", () => {
    ...
    document.getElementById("swapResult").textContent = isFinite(netTo)
      ? `≈ ${netTo.toFixed(4)} XAN (fee 0.3%)`
      : "—";
  });
}
...
function renderSparkline() {
  const pts = [50];
  for (let i = 1; i < 40; i++) {
    pts.push(Math.max(10, Math.min(90, pts[i - 1] + (Math.random() * 10 - 5))));
  }
  const d = pts
    .map((y, i) => `${(i / (pts.length - 1)) * 100},${100 - y}`)
    .join(" ");
  return `
...
function renderCrypto() {
  ...
  if (activeChip) {
    rows = rows.filter(
      (a) => (a.tags || []).includes(activeChip) || a.sector === activeChip
    );
  }
  if (onlyWatchlist) {
  ...
  rows.forEach((a) => {
    ...
    tdPrice.textContent = a.price != null ? `$${formatNum(a.price)}` : "-";

    const tdChg = document.createElement("td");
    tdChg.className = "num " + pctClass(a.change24h);
    tdChg.textContent =
      a.change24h != null ? `${a.change24h.toFixed(2)}%` : "-";

    const tdMc = document.createElement("td");
    tdMc.className = "num";
    tdMc.textContent = formatNum(a.marketCap);

    const tdFd = document.createElement("td");
    tdFd.className = "num";
    tdFd.textContent = formatNum(a.fdv);

    const tdVol = document.createElement("td");
    tdVol.className = "num";
    tdVol.textContent = formatNum(a.volume24h);
...
function renderFundraising() {
  ...
  if (items.length === 0) {
    fundraisingEmpty?.classList.remove("hidden");
    return;
  } else {
...
async function boot() {
  ...
  try {
    ...
    if (!assetsRes.ok)
      throw new Error("Assets JSON not found: " + assetsRes.status);
    if (!fundRes.ok)
      throw new Error("Fundraising JSON not found: " + fundRes.status);

    assets = await assetsRes.json();
    fundraising = await fundRes.json();

    // pastikan XAN punya logo & badge
    assets = assets.map((a) =>
      a.symbol === "XAN"
        ? {
            ...a,
    ...
    document.body.insertAdjacentHTML(
      "beforeend",
      `<pre style="color:#f88;background:#220;padding:8px;border:1px solid #400;border-radius:8px;max-width:90vw;overflow:auto">[Demo Error] ${String(
        e
      )}</pre>`
    );
  }
