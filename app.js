...
function openAssetDetail(asset){
  ...
  swapBtn.onclick = ()=>{
    ...
    const netTo = grossTo * (1 - 0.003);
    swapRes.textContent = isFinite(netTo) ? `≈ ${netTo.toFixed(4)} XAN (fee 0.3%)` : "—";
  };

  // Show detail, hide lists (full view)
...
function renderCrypto(){
  ...
  if (rows.length === 0){ cryptoEmpty.classList.remove("hidden"); return; }
  ...
  rows.forEach(a=>{
    ...
    wrap.appendChild(nm); tdName.appendChild(wrap);
...
function renderFundraising(){
  ...
  items.forEach(f=>{
    ...
    (f.tags||[]).forEach(t=>{
      const s = document.createElement("span"); s.className="tag"; s.textContent = t; right.appendChild(s);
    });
